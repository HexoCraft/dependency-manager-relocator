...
public class DMRelocator {
    ...
    private File libDir = new File("./libs/relocated").getAbsoluteFile();
    // The folder which will contains downloaded dependencies
    private File cacheDir = new File("./libs/cache").getAbsoluteFile();
     ...
     */
    public DMRelocator libDir(File folder) {
        this.libDir = folder.getAbsoluteFile();
        return this;
    }
     ...
     * @return instance of DMRelocator
     */
    public DMRelocator cacheDir(File cache) {
        this.cacheDir = cache.getAbsoluteFile();
        return this;
    }
    ...
    public DMRelocator relocate() throws IOException {
        ...
        Downloader.download(asmArtifact, repositories, cacheDir.toPath());
        Downloader.download(asmCommonsArtifact, repositories, cacheDir.toPath());
        Downloader.download(jarRelocatorArtifact, repositories, cacheDir.toPath());

        // Download artifacts
        for (Artifact artifact : artifacts) {
            Downloader.download(artifact, repositories, cacheDir.toPath());
        }

        // Inject DMRelocator dependencies
        // (asm, asm-commons and jar-relocator)
        UrlClassLoader.addToClassLoader(classLoader, cacheDir.toPath().resolve(asmArtifact.toPath()).toFile());
        UrlClassLoader.addToClassLoader(classLoader, cacheDir.toPath().resolve(asmCommonsArtifact.toPath()).toFile());
        UrlClassLoader.addToClassLoader(classLoader, cacheDir.toPath().resolve(jarRelocatorArtifact.toPath()).toFile());

        // Relocate and inject dependencies
        for (Artifact artifact : artifacts) {
            Relocator.relocate(classLoader, artifact, relocations, cacheDir.toPath(), libDir.toPath());
        }
